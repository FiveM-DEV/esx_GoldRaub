...
Citizen.CreateThread(function()
	...
	while true do
		...
		TriggerClientEvent("esx_goldRaub:spawnNPC",-1,Config.MissionNPC[NPC])
		Citizen.Wait(7200000*2)
	end
end)

AddEventHandler('esx:playerLoaded', function(playerId, xPlayer)
	TriggerClientEvent("esx_goldRaub:spawnNPC",playerId,Config.MissionNPC[NPC])
end)

-- server side for cooldown timer
RegisterServerEvent("esx_goldRaub:MeltingCooldown")
AddEventHandler("esx_goldRaub:MeltingCooldown",function(source)
	table.insert(SmelteryTimer,{MeltingTimer = GetPlayerIdentifier(source), time = ((Config.SmelteryTime * 1000))})
end)

-- server side for cooldown timer
RegisterServerEvent("esx_goldRaub:ExhangeCooldown")
AddEventHandler("esx_goldRaub:ExhangeCooldown",function(source)
	table.insert(ExchangeTimer,{ExchangeTimer = GetPlayerIdentifier(source), timeExchange = ((Config.ExchangeCooldown * 60000))})
end)

-- server side for cooldown timer
RegisterServerEvent("esx_goldRaub:GoldJobCooldown")
AddEventHandler("esx_goldRaub:GoldJobCooldown",function(source)
...
ESX.RegisterServerCallback("esx_goldRaub:getGoldJobCoolDown",function(source,cb)
...
RegisterServerEvent('esx_goldRaub:missionAccepted')
AddEventHandler('esx_goldRaub:missionAccepted', function()
	TriggerClientEvent("esx_goldRaub:startMission",source,0)
end)

ESX.RegisterServerCallback("esx_goldRaub:getPayment",function(source,cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	...
	if Config.UseBlackMoneyAsMissionCost == true then
		if blackMoney <= Config.MissionCost then
			...
			TriggerEvent("esx_goldRaub:GoldJobCooldown",source)
			cb(true)
		end
	else
...
ESX.RegisterServerCallback("esx_goldRaub:getMissionavailability",function(source,cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local Players = ESX.GetPlayers()
...
-- mission reward
RegisterServerEvent('esx_goldRaub:reward')
AddEventHandler('esx_goldRaub:reward', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	...
	if Config.EnableSecondItemReward == true and SecondItem == true then
		...
		if Config.EnableCustomNotification == true then
			TriggerClientEvent("esx_goldRaub:missionComplete", source,itemAmount1,item1,itemAmount2,item2)
		else
		...
		xPlayer.addInventoryItem(Config.ItemName1,itemAmount1)
		if Config.EnableCustomNotification == true then
			TriggerClientEvent("esx_goldRaub:missionComplete", source,itemAmount1,item1)
		else
...
RegisterServerEvent('esx_goldRaub:GoldJobInProgress')
AddEventHandler('esx_goldRaub:GoldJobInProgress', function(targetCoords, streetName)
	TriggerClientEvent('esx_goldRaub:outlawNotify', -1,string.format("^3 Shots fired ^0 at ^5%s^0 and ongoing grand theft auto",streetName))
	TriggerClientEvent('esx_goldRaub:GoldJobInProgress', -1, targetCoords)
end)

-- sync mission data
RegisterServerEvent("esx_goldRaub:syncMissionData")
AddEventHandler("esx_goldRaub:syncMissionData",function(data)
	TriggerClientEvent("esx_goldRaub:syncMissionData",-1,data)
end)

-- server side function for converting part
RegisterServerEvent('esx_goldRaub:goldMelting')
AddEventHandler('esx_goldRaub:goldMelting', function()
	...
	if xPlayer.getInventoryItem("goldwatch").count >= 100 then
		if xPlayer.getInventoryItem("goldbar").count <= 99 then
			if not CheckIfMelting(GetPlayerIdentifier(source)) then
				TriggerEvent("esx_goldRaub:MeltingCooldown",source)
...
RegisterServerEvent('esx_goldRaub:goldExchange')
AddEventHandler('esx_goldRaub:goldExchange', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	local _source = source
	
	if not CheckIfExchanging(GetPlayerIdentifier(source)) then
		if xPlayer.getInventoryItem("goldbar").count >= 70 then
			TriggerEvent("esx_goldRaub:ExhangeCooldown",source)
