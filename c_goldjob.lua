...
RegisterNetEvent('esx_goldRaub:outlawNotify')
AddEventHandler('esx_goldRaub:outlawNotify', function(alert)
...
RegisterNetEvent("esx_goldRaub:startMission")
AddEventHandler("esx_goldRaub:startMission",function(spot)
	...
	if numy == 100 then
		...
		CurrentMission = num
		TriggerEvent("esx_goldRaub:startTheEvent",num)
		PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
	end
	talkingWithNPC = false
end)

RegisterNetEvent('esx_goldRaub:GoldJobInProgress')
AddEventHandler('esx_goldRaub:GoldJobInProgress', function(targetCoords)
...
RegisterNetEvent("esx_goldRaub:missionComplete")
AddEventHandler("esx_goldRaub:missionComplete", function(itemAmount1,item1,itemAmount2,item2)
...
RegisterNetEvent("esx_goldRaub:missionFailDeath")
AddEventHandler("esx_goldRaub:missionFailDeath", function()
	SetAudioFlag("LoadMPData", true)
    PlaySoundFrontend(-1, "RACE_PLACED", "HUD_AWARDS", 1)
...
RegisterNetEvent("esx_goldRaub:spawnNPC")
AddEventHandler("esx_goldRaub:spawnNPC",function(NPC)
...
function requestMissionFromNPC()
	...
	ESX.TriggerServerCallback('esx_goldRaub:getGoldJobCoolDown', function(cooldownTimer)
		...
		if not cooldownTimer then
			...
			ESX.TriggerServerCallback('esx_goldRaub:getMissionavailability', function(missionPossible)
				if missionPossible then
					ESX.TriggerServerCallback('esx_goldRaub:getPayment', function(payment)
						if payment then
							TriggerServerEvent("esx_goldRaub:missionAccepted")	
						else
							talkingWithNPC = false
						end
					end)					
...
-- Core Mission Part
RegisterNetEvent('esx_goldRaub:startTheEvent')
AddEventHandler('esx_goldRaub:startTheEvent', function(num)
	local Goons = {}
	local loc = Config.MissionPosition[num]
	Config.MissionPosition[num].InUse = true
	local playerped = GetPlayerPed(-1)
	
	TriggerServerEvent("esx_goldRaub:syncMissionData",Config.MissionPosition)
	...
	while not JobCompleted and not StopMission do
		...
		if JobInProgress == true then
			...
			if vanIsDelivered == true then	
				TriggerServerEvent("esx_goldRaub:reward")
			...
			if StopMission == true then
				...
				if Config.EnableCustomNotification == true then
					TriggerEvent("esx_goldRaub:missionFailDeath")
				else
...
function LockpickVanDoor()
	...
	if Config.PoliceNotfiyEnabled == true then
		TriggerServerEvent('esx_goldRaub:GoldJobInProgress',GetEntityCoords(PlayerPedId()),streetName)
	end
...
-- Sync mission data
RegisterNetEvent("esx_goldRaub:syncMissionData")
